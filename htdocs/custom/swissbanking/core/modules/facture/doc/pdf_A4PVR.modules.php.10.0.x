 ...
 * Copyright (C) 2014-2020		Reto Kessler		<reto.kessler@mercurylabs.ch>
 *
 * Licence       : COMMERCIAL
 */

/**
 *	\file       htdocs/custom/swissbanking/core/modules/facture/doc/pdf_A4PVR.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from A4PVR model
 */
...
/**
 *	Class to manage PDF invoice template A4PVR
 */
class pdf_A4PVR extends ModelePDFFactures
{
     /**
  ...
  private function modulo10($number) {
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "A4PVR";
		$this->description = $langs->trans('PDFA4PVRDescription');
    ...
    $this->pvr_delta = 297 - $conf->global->SWISSBANKING_PAGEFORMATY;
		...
		$this->posxdesc=$this->marge_gauche+6;
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
                ...
                $heightforinfotot = 38+(4*$nbpayments);	// Height reserved to output the info and total part and payment part
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset($object->ref). 'with Mercurylabs SAGL PVR');
				$pdf->SetSubject($outputlangs->transnoentities("PdfInvoiceTitle"));
				$pdf->SetCreator('Mercury Labs SAGL Invoice PVR PDF Generator');
        ...
        if(isset($conf->global->SWISSBANKING_ISRBGPRINT) && $conf->global->SWISSBANKING_ISRBGPRINT == 'Yes'){
          ...
          $pdf->Image(DOL_DOCUMENT_ROOT.'/custom/swissbanking/img/ISR_Background.png', $conf->global->SWISSBANKING_ISRBGX, $conf->global->SWISSBANKING_ISRBGY + $this->pvr_delta, $conf->global->SWISSBANKING_PAGEFORMATX, $bvrbackgroundheight, 'PNG', '', '', true, 300, '', false, false, 0, false, false, true);
				...
				if ($notetoshow)
				{
					...
					// Rect prend une longueur en 3eme param
					$pdf->SetDrawColor(192, 192, 192);
					$pdf->Rect($this->marge_gauche, $tab_top-1, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $height_note);
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht-3, 3, $total_excl_tax, 0, 'R', 0);
          ...
          // retrieve global local tax
					...
					if (! empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblignes - 1))
					{
						...
						$pdf->line($this->marge_gauche + 5, $nexY+0.5, $this->page_largeur - $this->marge_droite - 5, $nexY+0.5);
        ...
        if($printdetail == 'IBAN') $printacc = trim(chunk_split(str_replace(" ","",str_replace(".","",str_replace(",","",$account->iban_prefix))), 4, ' '));
...
'.$account->proprio.'
'.$account->adresse_proprio.$account->owner_address;
        }
...
'.$account->adresse_proprio.$account->owner_address;
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
        ...
        $posx=$this->page_largeur-$this->marge_droite-$w+5;
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO))
		{
			if ($this->emetteur->logo)
			{
				...
				if (is_readable($logo))
				{
					...
					$pdf->Image($logo, $this->marge_gauche + 5, $posy, 0, $height);	// width=0 (auto)
				}
				else
				{
					$pdf->SetTextColor(200, 0, 0);
		...
		{
			$top_shift = $pdf->getY() - $current_y - 4;
		}
		...
		if ($showaddress)
		{
			...
			// Show sender
			$posy=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 52;
			$posy+=$top_shift;
			$posx=$this->marge_gauche + 5;
			if (! empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx=$this->page_largeur-$this->marge_droite-90;
			...
			$posy=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 52;
			$posy+=$top_shift;
			$posx=$this->page_largeur-$this->marge_droite-$widthrecbox-5;
