 ...
 * Copyright (C) 2014-2021		Reto Kessler		<reto.kessler@mercurylabs.ch>
 *
 * Licence       : COMMERCIAL
 */

/**
 *	\file       htdocs/custom/swissbanking/core/modules/facture/doc/pdf_Swissbanking1Page.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from Swissbanking1Page model
 */
...
require_once $dol_document_root . '/custom/swissbanking/includes/SwissQR/vendor/autoload.php';

/**
 *	Class to manage PDF invoice template Swissbanking1Page
 */
class pdf_Swissbanking1Page extends ModelePDFFactures
{
	 /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Swissbanking1Page";
		$this->description = $langs->trans('PDFSwissbanking1PageDescription');
		...
		$this->marge_droite = 5;
		$this->marge_haute = 5;
		if ($conf->global->SWISSBANKING_SLIPTYPE == 'QR') $this->marge_basse = 110;
		else $this->marge_basse = 111;
		$this->pvr_delta = 297 - $conf->global->SWISSBANKING_PAGEFORMATY;
	...
	{
		// phpcs:enable
		global $user, $langs, $conf, $mysoc, $hookmanager, $nblines, $db;
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset($object->ref).' created with Mercurylabs SAGL SwissBanking');
				$pdf->SetSubject($outputlangs->transnoentities("PdfInvoiceTitle"));
				$pdf->SetCreator('Mercury Labs SAGL SwissBanking Invoice PDF Generator');
        ...
        /* BEGIN ISR Slip, here is where the magic happens */
        // var_dump($object);
        require_once(DOL_DOCUMENT_ROOT."/core/lib/files.lib.php");

        $invoiceid = $object->id;
        $reference = $object->ref;
        $customercode = $object->client->code_client;

        if (!isset($_POST['pvrbankaccount'])){
        ...
        else $pvrbankaccount = GETPOST('pvrbankaccount',$check);
        ...
        $company = $object->thirdparty->name;
        $address = $object->thirdparty->address;
        $zip = $object->thirdparty->zip;
        $town = $object->thirdparty->town;
        $region = $object->thirdparty->state;
        $regioncode = $object->thirdparty->state_code;
        $country = $object->thirdparty->country;
        $countrycode = $object->thirdparty->country_code;
        $invoicetotal = $object->total_ttc;
        ...
        $sql .= " WHERE re.fk_facture_source = f.rowid AND re.fk_facture = ".((int) $invoiceid);
        $resql = $this->db->query($sql);
        if ($resql) {
        ...
        $sql .= " WHERE pf.fk_paiement = p.rowid AND pf.fk_facture = ".((int) $invoiceid);
        ...
        if ($resql) {
          ...
          while ($i < $num) {
            ...
            $invoicetotal = $invoicetotal - (($conf->multicurrency->enabled && $object->multicurrency_tx != 1) ? $row->multicurrency_amount : $row->amount);
            $i++;
          }
        }
        else {
        ...
        if ($conf->global->SWISSBANKING_SLIPTYPE == 'QR') { // Generation of the new Swiss QR Bill
          ...
          $QRRefNum = str_pad(preg_replace('/[^0-9]*/','', $customercode).$invoiceid.preg_replace('/[^0-9]*/','', $reference), 20 ,'0', STR_PAD_LEFT);
          ...
          try {
            $outputdirectory = $conf->facture->dir_output . '/' . $reference . '/';
            $qrBill->getQrCode()->writeFile($conf->facture->dir_output . '/' . $reference . '/qr.png');
          }
          catch (Exception $e) {
          ...
          $pdf->Image($conf->facture->dir_output . '/' . $reference . '/qr.png', $xpos, $ypos + 7 + 5, 46, 46, 'PNG', '', '', true, 300, '', false, false, 0, false, false, false);
          ...
          if (file_exists($conf->facture->dir_output . '/' . $reference . '/qr.png')) unlink($conf->facture->dir_output . '/' . $reference . '/qr.png');
        }
...
'.$account->owner_address;
          ...
          // Set ISR Background if necessary
          if(isset($conf->global->SWISSBANKING_ISRBGPRINT) && $conf->global->SWISSBANKING_ISRBGPRINT == 'Yes'){
            ...
            $pdf->Image(DOL_DOCUMENT_ROOT . '/custom/swissbanking/img/ISR_Background.png', $conf->global->SWISSBANKING_ISRBGX, $conf->global->SWISSBANKING_ISRBGY + $this->pvr_delta, $conf->global->SWISSBANKING_PAGEFORMATX, $bvrbackgroundheight, 'PNG', '', '', true, 300, '', false, false, 0, false, false, true);
          }
          ...
          $ypos = $conf->global->SWISSBANKING_BANKADDRESSLEFTY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_BANKADDRESSRIGHTY + $this->pvr_delta;
          $pdf->SetFont($conf->global->SWISSBANKING_PRINTCHARACTER, '', $default_font_size - 2);
          $pdf->SetXY($xpos, $ypos);
          $pdf->MultiCell(0, 0, $bankaddress, 0, 'L');
          ...
          if(stripos($account->bank, 'post') === False){
            ...
            $ypos = $conf->global->SWISSBANKING_COMPANYADDRESSLEFTY + $this->pvr_delta;
          }
          else{
            $xpos = $conf->global->SWISSBANKING_BANKADDRESSLEFTX;
            $ypos = $conf->global->SWISSBANKING_BANKADDRESSLEFTY + $this->pvr_delta;
          ...
          if(stripos($account->bank, 'post') === False){
            ...
            $ypos = $conf->global->SWISSBANKING_COMPANYADDRESSRIGHTY + $this->pvr_delta;
          }
          else{
            $xpos = $conf->global->SWISSBANKING_BANKADDRESSRIGHTX;
            $ypos = $conf->global->SWISSBANKING_BANKADDRESSRIGHTY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_BANKACCOUNTLEFTY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_BANKACCOUNTRIGHTY + $this->pvr_delta;
          $pdf->SetFont('OCRB10', '', 10);
          $pdf->SetXY($xpos, $ypos);
          $pdf->Cell(0, 0, $ccp, 0, 'L');

          // Calculate Upper Reference Number
          $referencenumber = str_pad(preg_replace('/[^0-9]*/', '', $customercode) . $invoiceid . preg_replace('/[^0-9]*/', '', $reference), 20 ,'0', STR_PAD_LEFT);
          ...
          $ypos = $conf->global->SWISSBANKING_PVRREFERENCEY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_LEFTADDRESSY - 3 + $this->pvr_delta;
          $pdf->SetFont($conf->global->SWISSBANKING_PRINTCHARACTER, '', 6);
          $pdf->SetXY($xpos, $ypos);
          $pdf->Cell(0, 0, $upperreferencenumber, 0, 'L');

          //create bottom line string
          if($totalprint <= 0) $lowerreferencenumber = "042>";
          else{
          ...
          $ypos = $conf->global->SWISSBANKING_PVRM10Y + $this->pvr_delta;
          $pdf->SetFont('OCRB10', '', 10);
          $pdf->SetXY(7, $ypos);
          ...
          $address = explode("\n", $address);
          $xpos = $conf->global->SWISSBANKING_LEFTADDRESSX;
          $ypos = $conf->global->SWISSBANKING_LEFTADDRESSY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_RIGHTADDRESSY + $this->pvr_delta;
          ...
          $ypos = $conf->global->SWISSBANKING_LEFTDIGITSMY + $this->pvr_delta;
          $pdf->SetXY($xpos, $ypos);
          ...
          $ypos = $conf->global->SWISSBANKING_LEFTDIGITSCY + $this->pvr_delta;
          $pdf->SetXY($xpos, $ypos);
          for($i = 0; $i <= 2; $i++){
          ...
          $ypos = $conf->global->SWISSBANKING_RIGHTDIGITSMY + $this->pvr_delta;
          $pdf->SetXY($xpos, $ypos);
          ...
          $ypos = $conf->global->SWISSBANKING_RIGHTDIGITSCY + $this->pvr_delta;
          $pdf->SetXY($xpos, $ypos);
          for($i = 0; $i <= 2; $i++){
            $pdf->Cell(0 ,0, $centstotal[$i], 0, 'L');
        ...
        if($invoiceid != ''){
          $sql = "SELECT * FROM ".MAIN_DB_PREFIX."swissbanking WHERE invoiceid = '".$invoiceid."' LIMIT 1";
          $resql = $this->db->query($sql);
          if ($resql) {
            if($this->db->num_rows($resql)) $sql = "UPDATE ".MAIN_DB_PREFIX."swissbanking SET isrcode='".$referencenumber."',amount='".$invoicetotal."',lastgenwith='".$pvrbankaccount."',status='open' WHERE invoiceid='".$invoiceid."'";
        ...
        /* END ISR Slip, here is where the magic happened */
